...
public class AdminMenu extends Menu {
	...
	protected void addBook() {
		...
		bookManager.addBook(title, isbn, page, Collections.singletonList(author), publisher, price);
		Helper.prompt("Added book successfully!");
	}

	private String getBookId(String s) {
		String bookId = Helper.getString(() -> Helper.print("Book id to " + s + "(0 to cancel): "));
		if (bookId.equals("0")) {
	...
	protected void listUser() {
		...
		Helper.printTable(new String[] { "ID", "Name", "Email" }, "=", userManager.listUser()
				.map(user -> new String[] { user.getId(), user.getName(), user.getEmail(), }).toArray(String[][]::new),
				new int[] { 50, 10, 30 }, "| ");
		Helper.prompt();
	}
	...
	private String getUserId(String s) {
		String userId = Helper.getString(() -> Helper.print("User id to " + s + "(0 to cancel): "));
		if (userId.equals("0")) {
			Helper.prompt("Canceled " + s + "!");
			return null;
		}
